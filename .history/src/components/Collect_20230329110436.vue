<template>
  <div v-if="this.isEmpty" class="container">还没有收藏视频</div>
...
<script>
import { getCollectList } from "../services/video";
...
export default {
  ...
  methods: {
    async getMovList() {
      try {
        const res = await getCollectList(this.pageNum, this.pageSize);
  ...
  created() {
    this.$watch("currentPage", async (newValue, oldValue) => {
      ...
      try {
        const res = await getCollectList(this.currentPage, this.pageSize);
...
body {
  height: 100%;
  background-image: url(./assets/pexels-madison-inouye-1831234.jpg);
