...
<script>
import { getHistoryList } from "../services/video";
...
export default {
  ...
  methods: {
    async getMovList() {
      try {
        const res = await getHistoryList(this.pageNum, this.pageSize);
  ...
  created() {
    this.$watch("currentPage", async (newValue, oldValue) => {
      ...
      try {
        const res = await getHistoryList(this.currentPage, this.pageSize);
